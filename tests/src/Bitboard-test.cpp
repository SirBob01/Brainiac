...
static char *test_find_lsb_bitboard() {
    ...
    mu_assert("Find lsb bits (4)", find_lsb_bitboard(b1) == 4);
...
static char *test_count_set_bitboard() {
    Bitboard b0 = 1;
    mu_assert("Count set bits (1)", count_set_bitboard(b0) == 1);

    Bitboard b1 = 0b1100111010010;
    mu_assert("Count set bits (7)", count_set_bitboard(b1) == 7);

    Bitboard b2 = 0;
    mu_assert("Count set bits (0)", count_set_bitboard(b2) == 0);
    return 0;
}

static char *test_flip_vertical_bitboard() {
    Bitboard b0 = 1;
    mu_assert("Flip vertical",
...
static char *test_get_lsb_bitboard() {
    Bitboard b0 = 1;
    mu_assert("Get LSB (1)", get_lsb_bitboard(b0) == 1);

    Bitboard b1 = 0b1100111010000;
    mu_assert("Get LSB (10000)", get_lsb_bitboard(b1) == 0b10000);
...
static char *test_pop_lsb_bitboard() {
    Bitboard b0 = 1;
    mu_assert("Pop LSB (1)", pop_lsb_bitboard(b0) == 0);

    Bitboard b1 = 0b1100111010010;
    mu_assert("Pop LSB (7)", pop_lsb_bitboard(b1) == 0b1100111010000);

    Bitboard b2 = 0;
    mu_assert("Pop LSB (0)", pop_lsb_bitboard(b2) == 0);
    return 0;
}
