...
namespace chess {
    ...
    uint64_t zobrist_hash(Color turn, uint64_t *bitboards, uint8_t castling_rights, Square en_passant) {
        ...
        for(int color = Color::White; color <= Color::Black; color++) {
            for(int type = 0; type < PieceType::NPieces; type++) {
                ...
                for(int i = 0; i < 64; i++) {
                    if(bitboard & 1) {
                        r ^= piece_bitstrings[(i * PieceType::NPieces * 2) + index];
                    }
                    bitboard >>= 1;
                }
            }
    ...
    uint64_t zobrist_bitstring(Piece piece, Square square) {
        ...
        return piece_bitstrings[(square.shift * PieceType::NPieces * 2) + piece_index];
    }
}